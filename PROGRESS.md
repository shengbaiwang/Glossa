...
- 按用户确认的 A 方案，书内搜索统一用「搜索」，占位文字固定为「搜索书内文字」，入口固定为「选项」。删除高级页及重复全词开关，单层「方式」列出普通 / 全词 / 邻近 / 正则，分隔后为区分大小写、区分变音符号；邻近方式才显示同菜单内的 5/10/20/50 词距离，非默认时提供恢复默认。保留模式兼容、独立开关、正则下变音禁用及原搜索流程。
