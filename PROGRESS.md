...
### 2026-09-09 独立右侧笔记栏开发建议

- 根据用户对“右侧独立笔记栏与 LLM 阅读重点”的咨询，核对当前代码：`ReaderContent` 已挂载右侧 `Notebook`，支持固定、收起与调宽；`StudyNotesPanel` 仍由左侧 `sidebar/Content` 挂载。已有按章提取、来源验证/跳转、版本、个人补充与本地 IndexedDB 存储可复用。
- 建议先接入右侧 Notebook，统一学习笔记与摘录/心得入口，再增加“少量核心观点 → 完整论述 → 来源核对”的分层呈现。当前长章生成保留分段详情但不产出全章统合概览；后续重点层应保留原始来源映射并回查证据，不能仅反复压缩分段摘要。章节跟随仅切换已有笔记；主动生成和处理范围边界继续保留。
- 建议的实施顺序与验收方法属于产品咨询，尚未修改应用或将新增建议写为 `PLAN.md` 的已定实施范围。AI 学习笔记目前仅本地保存；统一界面不意味着已接入云同步。下一步最小交付建议为 EPUB 右侧栏接入、核心内容折叠及来源返回流程，随后用少量真实章节评估观点覆盖、忠实度和使用效率。
- 会话内交互示意使用原创演示文章与静态笔记，未调用 LLM、读取密钥或发送用户正文。Chromium 的 1024/736/360px 浅深色布局与展开、来源高亮/返回、标签切换、草稿保留、收起检查均通过，已查看桌面截图；首次浏览器启动受沙箱 MachPort 权限限制，获准隔离启动后通过。本次仅更新进度记录，不运行产品构建或产品回归；保留其他任务同时修改的透明度代码、测试与记录。

### 2026-09-09 外观透明度

- 按用户要求，将背景图片、高亮和阅读标尺的调节项统一从“不透明度”改为“透明度”，同步简繁中文文案和可访问名称。背景滑块显示 0–100%、步长 5%，向右增加透明度。
- 当前处于开发阶段，不保留旧 `*Opacity` 设置字段或旧值换算：设置直接以 `*Transparency` 保存。仅在交给浏览器 CSS `opacity` 属性或阅读标尺渲染时计算 `1 - transparency`；默认视觉效果保持不变。
- 新增控件测试覆盖背景、高亮和阅读标尺的透明度读写及背景边界。Node 24.19.0 下的相关 5 个测试文件、166 项测试通过；TypeScript、修改文件的 Biome 检查与 `git diff --check` 通过。背景纹理 hook 的单独测试依赖本机 Supabase 测试环境，未加载该环境时初始化失败，未将其记为通过。
- 源码交付完成；下一步：后续原生打包时纳入本次修改，当前未重新打包或替换已安装应用。
