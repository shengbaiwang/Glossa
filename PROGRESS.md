...
- 按用户要求，将背景图片控件从“不透明度”改为“透明度”，同步简繁中文文案与滑块可访问名称。滑块显示 0–100%、步长 5%，向右增加透明度；原 55% 不透明度显示为 45% 透明度。
- 仅在控件边界换算，继续按原有 opacity 字段保存并渲染，保留书库/阅读页作用范围、同步协议及已有背景效果。打开设置不改写旧值。
- 新增 7 项测试覆盖旧值转换、两端边界、操作写回与无背景状态；Node 24.19.0 加载项目测试环境和既有 canvas 的 NODE_PATH 后，全量测试 570 文件 / 7,000 项通过，3 文件 / 7 项既有跳过。TypeScript、全量 Biome lint（1,582 文件）、修改代码格式检查、翻译 JSON 校验与 `git diff --check` 通过。
- 初次定向测试未加载环境，背景 hook 因缺少 Supabase 基础配置初始化失败；初次全量使用系统 Node 26 出现 localStorage 兼容失败，已停止本轮测试并切换 Node 24，最终全量通过。未修改依赖或用户数据。
