...
- 书签列表重建为专属 `BookmarkView` + `BookmarkItem`（左栏书签标签页换用，`Content.tsx` 同步替换）：摘录为主内容，页码与相对时间常显（页码标签改用已翻译的 `Page: {{number}}`，替代旧的未翻译 `p {{page}}`），复制链接/内联改名/删除悬停可达且图标统一主题墨色不带语义彩色，行键改用记录 id；保留章节分组、CFI 排序（新增安全比较器）、虚拟滚动、OverlayScrollbars 初始化后回正与远距瞬时滚动。`BooknoteView` 与 `BooknoteItem` 移除书签分支改为批注专用（Notebook 调用同步去参），书签存储仍为 config.booknotes 的 BookNote 记录，同步协议与旧数据完全兼容，不迁移不清理。
- 验证：先以 16 项失败测试复现旧实现问题（错误摘录、前缀误匹配、重复书签、滞后丝带、自我修复、空范围回退等），再实现。最终书签专项 5 文件 67 项通过；阅读器组件/钩子/工具与顶栏回归 47 文件 387 项通过；全量单测 **614 文件 / 7,415 项通过，3 文件 / 7 项跳过**（168.5 秒）。`tsgo --noEmit`、全量 Biome lint（1,695 文件）、14 个变更文件 check 与 `git diff --check` 通过。期间修正测试用 CFI 夹具为 foliate 规范区间格式（偶数步带 offset 会在字符串往返中丢失），并先修复测试自身 Set 用法错误，未放宽断言。
