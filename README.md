...
Glossa 是基于 [Readest](https://github.com/readest/readest) 修改的开源阅读器，提供基础阅读、本地书库、书签批注、简单对话、选段导图和云端同步。Glossa 是独立项目；上游作者与贡献者的署名和开源许可保留在 [NOTICE](NOTICE)、[LICENSE](LICENSE) 和 [第三方声明](THIRD_PARTY_NOTICES.md) 中。

**修改声明（2026-09-13）：** 本项目基于 Readest v0.12.1，基线提交为 `f3e1df7e0572c0119cbb420e1e27ca9af859f91c`。Glossa 修改了应用名称、图标、独立运行身份、阅读界面及功能范围，将 AI 收缩为简单对话与选段导图，并移除了旧导读、通用聊天、翻译词典、朗读速读、订阅、公开分享和遥测等附加实现。具体差异见 Git 历史和 [PROGRESS.md](PROGRESS.md)。
...
- EPUB 导图：明确选择阅读段生成关系树，点击分支跳回已核验来源。
...
在“设置 → 模型服务”配置 API 地址、密钥与模型。右侧默认打开“对话”，每次只自动携带书名、作者和目录路径；“导图”需明确选择章节与阅读段后主动生成，点击分支文字可跳转原文并返回。历史保存在本机，暂不参与云同步。
