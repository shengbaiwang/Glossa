...
Glossa 是基于 [Readest](https://github.com/readest/readest) 修改的开源阅读器，提供基础阅读、本地书库、书签批注、简单对话、基础导图和云端同步。Glossa 是独立项目；上游作者与贡献者的署名和开源许可保留在 [NOTICE](NOTICE)、[LICENSE](LICENSE) 和 [第三方声明](THIRD_PARTY_NOTICES.md) 中。

**修改声明（2026-09-13）：** 本项目基于 Readest v0.12.1，基线提交为 `f3e1df7e0572c0119cbb420e1e27ca9af859f91c`。Glossa 修改了应用名称、图标、独立运行身份、阅读界面及功能范围，将 AI 收缩为简单对话与所选阅读段的导图生成，整合导图基础编辑和原文核对，并移除了旧导读、通用聊天、翻译词典、朗读速读、订阅、公开分享和遥测等附加实现。具体差异见 Git 历史和 [PROGRESS.md](PROGRESS.md)。
...
- 书签、高亮、笔记、批注导出及跳回原文。
- 简单 API 对话：自动附带书名、作者与当前目录路径，支持模型切换和本机历史。
- EPUB 导图：可基于明确选择的阅读段生成，也可手工整理观点；大纲与导图共用同一结构，支持编辑、折叠、撤销及备份恢复，点击有来源的观点核验后跳转并显示对应原文。
...
在“设置 → 模型服务”配置 API 地址、密钥与模型。右侧默认打开“对话”，每次只自动携带书名、作者和目录路径；“导图”在明确选择章节和阅读段后生成，结果可直接修改、核验原文并跳转，也支持无需模型的手工编辑。历史保存在本机，暂不参与云同步。

原生版 API 密钥使用系统安全存储，浏览器版仅在当前会话保留密钥。导读已移除，旧本地数据库不再使用；普通摘录、高亮和手写心得继续可用。
