...
**修改声明（2026-09-13）：** 本项目基于 Readest v0.12.1，基线提交为 `f3e1df7e0572c0119cbb420e1e27ca9af859f91c`。Glossa 修改了应用名称、图标、独立运行身份、阅读界面及功能范围，将 AI 收缩为简单对话，导图先完善独立于 AI 的基础工作区，并移除了旧导读、通用聊天、翻译词典、朗读速读、订阅、公开分享和遥测等附加实现。具体差异见 Git 历史和 [PROGRESS.md](PROGRESS.md)。
...
- EPUB 导图：无需模型即可整理观点，大纲与导图共用同一层级结构，支持编辑、折叠聚焦、撤销及备份恢复；旧生成图保留核验后跳转来源。
...
在“设置 → 模型服务”配置 API 地址、密钥与模型。右侧默认打开“对话”，每次只自动携带书名、作者和目录路径；“导图”无需模型即可新建并整理观点，旧生成图的分支文字仍可核验后跳转原文并返回。历史保存在本机，暂不参与云同步。
