...
// A failed or cancelled local save stays recoverable when the sidebar is reopened.
const unsavedGuides = new Map<string, ReadingGuide>();
...
export default function ReadingGuidePanel(props: Props) {
  return <ReadingGuideBook key={`${props.book.hash}:${props.bookKey}`} {...props} />;
}

function ReadingGuideBook(props: Props) {
  const { book, bookDoc, bookKey } = props;
  ...
  return (
    <div className='glossa-guide-panel' aria-label={_('Guide')}>
      ...
      <label className='glossa-guide-label' htmlFor={`${bookKey}-guide-chapter`}>
        {_('Chapter')}
      </label>
      <select
        id={`${bookKey}-guide-chapter`}
...
function ChapterGuide(props: ChapterProps) {
  const { bookDoc, bookKey, chapter } = props;
  const _ = useTranslation();
  ...
  return (
    <div className='glossa-guide-chapter'>
      <label className='glossa-guide-label' htmlFor={`${bookKey}-guide-passage`}>
        {_('Passage')}
      </label>
      <select
        id={`${bookKey}-guide-passage`}
        className='glossa-guide-select eink-bordered'
        value={passageId}
...
function PassageGuide({
  ...
  providerReady,
  openModels,
}: ChapterProps & { passage: ReadingPassage }) {
  const _ = useTranslation();
  const identity = JSON.stringify([book.hash, chapter.id, passage.id]);
  const [guide, setGuide] = useState<ReadingGuide | null>(
    () => unsavedGuides.get(identity) ?? null,
  );
  ...
  useEffect(() => {
    ...
    setReady(false);
    setLoadError('');
    void loadReadingGuide(book.hash, chapter.id, passage.id)
      .then((saved) => {
        if (!current) return;
  ...
  }, [book.hash, chapter.id, passage.id, identity, reload]);

  useEffect(() => {
    let current = true;
    setCacheKey('');
    if (config)
      void getReadingGuideCacheKey(book.hash, passage, config)
        .then((key) => {
  ...
  const persist = async (result: ReadingGuide, controller: AbortController) => {
    setBusy('saving');
    setNotice('');
    try {
      await saveReadingGuide(result, controller.signal);
