...
describe('TOCItem accessibility', () => {
  ...
  it('page number uses the quiet tabular style and keeps regular weight when active', () => {
    const item = makeLeafItem({ index: 2 });
    const { container } = render(
      <StaticListRow
        {...defaultProps}
    ...
    const pageDiv = container.querySelector('.glossa-reader-toc-page') as HTMLElement;
    expect(pageDiv).toBeTruthy();
    expect(pageDiv.textContent).toBe('3');
    // Monospaced digits, one step below the muted chrome, never bold.
    // (Color/size/weight live in the .glossa-reader-toc-page CSS rules.)
    expect(pageDiv.className).toContain('glossa-reader-toc-page');
    expect(pageDiv.className).toContain('font-mono');
  });
});
