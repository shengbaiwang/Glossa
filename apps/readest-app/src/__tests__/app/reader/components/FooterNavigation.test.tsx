...
describe('reader footer navigation', () => {
  ...
  it('keeps page turns beside the page field and separates history navigation', () => {
    const config = props();
    render(<DesktopFooterBar {...config} />);
  ...
  it('jumps sections on long-press of the page buttons without turning a page', () => {
    vi.useFakeTimers();
    const config = props();
    render(<DesktopFooterBar {...config} />);
    const pages = screen.getByRole('group', { name: 'Go to Page' });

    const prev = within(pages).getByRole('button', { name: 'Previous Page' });
    fireEvent.pointerDown(prev, { pointerId: 1, clientX: 10, clientY: 10 });
    vi.advanceTimersByTime(600);
    fireEvent.pointerUp(prev, { pointerId: 1, clientX: 10, clientY: 10 });
    fireEvent.click(prev);
    expect(config.navigationHandlers.onPrevSection).toHaveBeenCalledOnce();
    expect(config.navigationHandlers.onPrevPage).not.toHaveBeenCalled();

    const next = within(pages).getByRole('button', { name: 'Next Page' });
    fireEvent.pointerDown(next, { pointerId: 2, clientX: 10, clientY: 10 });
    vi.advanceTimersByTime(100);
    fireEvent.pointerUp(next, { pointerId: 2, clientX: 10, clientY: 10 });
    expect(config.navigationHandlers.onNextPage).toHaveBeenCalledOnce();
    expect(config.navigationHandlers.onNextSection).not.toHaveBeenCalled();
    vi.useRealTimers();
  });
