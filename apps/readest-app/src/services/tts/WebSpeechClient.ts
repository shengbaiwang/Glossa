...
import type { SpeechMarkSink } from './TTSClient';
...
async function* speakWithMarks(
  ...
  for (const mark of marks) {
    ...
    const result = await new Promise<TTSBoundaryEvent>((resolve) => {
      const finish = (event: TTSBoundaryEvent) => {
      ...
      const abort = () => {
        synth.cancel();
        finish({ type: 'error', speaking: false, error: 'Aborted' });
      };
      signal.addEventListener('abort', abort, { once: true });
      utterance.onend = () => finish({ type: 'end', speaking: false });
      utterance.onerror = (event) => finish({ type: 'error', speaking: false, error: event.error });
...
export class WebSpeechClient implements TTSClient {
  ...
  constructor(controller?: SpeechMarkSink) {
  ...
  async init() {
    ...
    await new Promise<void>((resolve) => {
      ...
      const populateVoices = () => {
        ...
        settled = true;
        clearTimeout(timer);
        this.#synth.removeEventListener?.('voiceschanged', populateVoices);
      ...
      // SpeechMarkSink.init(), which takes every other engine down with it —
  ...
  async setVoice(voiceId: string) {
    ...
    this.#preferredVoiceId = selectedVoice?.id || '';
    this.#currentVoiceId = this.#preferredVoiceId;
  }
