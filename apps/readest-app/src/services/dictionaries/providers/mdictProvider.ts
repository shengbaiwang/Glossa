import { sanitizeDictionaryHtml } from '../sanitize';
/**
 * MDict provider.
 *
      ...
      for (const mdd of mdds) {
        for (const key of candidates) {
          try {
            ...
            console.warn('Dictionary resource unavailable');
          }
        }
      }
    }),
...
async function wireMdictAudioOnclick(
  ...
  // Only a recognized, inert audio marker survives HTML sanitization.
  const elements = Array.from(container.querySelectorAll<HTMLElement>('[data-glossa-audio]'));
  for (const el of elements) {
    const onclick = el.getAttribute('data-glossa-audio') ?? '';
    ...
    el.removeAttribute('data-glossa-audio');
    ...
    async lookup(word, ctx) {
      ...
      try {
        ...
        const extractRedirect = (def: string | null): string | null => {
          if (!def) return null;
          redirectScratch.innerHTML = sanitizeDictionaryHtml(def);
        ...
        const inert = new DOMParser().parseFromString(
          `<body>${result.definition}</body>`,
          'text/html',
        );
        for (const element of inert.querySelectorAll('[onclick]')) {
          const handler = element.getAttribute('onclick') || '';
          if (V0R_PLAY_RX.test(handler)) element.setAttribute('data-glossa-audio', handler);
        }
        body.innerHTML = sanitizeDictionaryHtml(inert.head.innerHTML + inert.body.innerHTML);
