...
import {
  ChevronLeft as MdArrowBack,
  ChevronRight as MdChevronRight,
  SlidersHorizontal as MdSettings,
  Volume as MdVolumeUp,
  X,
...
export interface DictionaryResultsState {
  ...
  isSpeaking: boolean;
  /** Pronounce the current word via platform speech. */
  speakWord: () => void;
}
...
export function useDictionaryResults({
  ...
  // Platform playback so the header button can show one active state.
      ...
      <div className='flex min-w-0 flex-1 items-center justify-center gap-1'>
        {onSpeak ? (
          <button
            ...
            onClick={onSpeak}
            className={clsx(
              'glossa-icon-button touch-target shrink-0',
              speaking
